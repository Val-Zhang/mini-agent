...
4. **持久化**：todos 保存在 `.agent/todos.json`，跨会话保持状态。
...
src/tools/todo/
  ...
  state.ts       # TodoState 类，负责状态管理和持久化
...
**TodoState 类**：
- `load()`：从 `.agent/todos.json` 加载状态，文件不存在时初始化空状态。
- `save()`：保存状态到 JSON 文件，自动创建目录。
