...
在 `execute` 模式下，工具调用仍会先经过 permission pipeline：只读工具默认允许，`write_file` / `edit_file` 默认询问确认，危险 bash 命令会被拒绝。`/implement` 会带上已批准 plan 的上下文，允许执行计划中的文件写入。
计划草案会保存在 `memories/session/plan.md`，`/mode` 会显示当前计划状态和摘要。上下文压缩摘要会保存在 `memories/session/compact.md`，完整消息转录会追加到 `memories/session/transcript.jsonl`。默认 hook 会把 session 和工具执行审计记录追加到 `memories/session/audit.jsonl`。
