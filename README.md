# mini-agent

一个用 Node.js 从零构建的 terminal agent 练习项目。

这个仓库会优先保持“小而清楚”：先把终端入口、通用模型调用、消息循环、工具调用、上下文管理这些能力拆开写明白，再逐步补全成一个可用的 agent。

## 当前初始化内容

- TypeScript + Node.js ESM 项目
- `mini-agent` CLI 入口
- OpenAI-compatible 模型客户端骨架，本地模型优先，也预留商业 API 接入
- 简单的终端对话循环
- 基于 `node:test` 的基础测试

## 环境要求

- Node.js 20+
- 一个模型服务，推荐先使用本地 OpenAI-compatible 服务，也可以换成商业 API

常见选择：

- Ollama
- LM Studio
- llama.cpp server
- vLLM

## 快速开始

```bash
cp .env.example .env
npm start
```

CLI 会自动读取本地 `.env`。如果模型服务没有启动或 API 配置错误，CLI 会提示连接失败。后续我们会把模型 provider、工具层和 agent loop 拆得更细。

常用命令：

- `/plan`：切换到规划模式，只生成计划，不执行命令和写入类工具。
- `/approve`：批准当前 plan 草案。
- `/reject`：丢弃当前 plan 草案。
- `/implement`：执行已批准的 plan（自动进入 execute 模式）。
- `/execute`：切换到执行模式，允许正常工具执行。
- `/mode`：查看当前模式和 plan 状态。
- `/context`：查看当前上下文窗口估算占用率和 token 分类。
- `/compact`：手动压缩当前会话，把旧消息总结到 `memories/session/compact.md`。
- `/exit`：退出 CLI。

在 `plan` 模式下，agent 只会看到并允许只读规划工具（当前为 `read_file`、`todo_write`、`list_dir`、`glob`、`grep`、`web_fetch`、`load_skill`）。即使模型尝试调用写入/命令类工具，也会在执行层被拦截。
在 `execute` 模式下，工具调用仍会先经过 permission pipeline：只读工具默认允许，`write_file` / `edit_file` / `remember` 默认询问确认，危险 bash 命令会被拒绝。`/implement` 会带上已批准 plan 的上下文，允许执行计划中的文件写入。
计划草案会保存在 `memories/session/plan.md`，`/mode` 会显示当前计划状态和摘要。上下文压缩摘要会保存在 `memories/session/compact.md`，完整消息转录会追加到 `memories/session/transcript.jsonl`。默认 hook 会把 session 和工具执行审计记录追加到 `memories/session/audit.jsonl`。长期记忆由 `remember` 工具显式写入 `memories/project/facts.md`、`memories/project/workflow.md` 和 `memories/user/preferences.md`，并以小摘要形式注入模型上下文。

## 配置

通过环境变量配置模型 provider：

```bash
MODEL_PROVIDER=local-openai-compatible
MODEL_BASE_URL=http://localhost:11434/v1
MODEL_NAME=qwen2.5-coder:7b
MODEL_API_KEY=local
AGENT_MAX_TURNS=24
AGENT_TRACE=compact
AGENT_SUBAGENT_TRACE=compact
```

`MODEL_BASE_URL` 应指向 OpenAI-compatible API 的根路径，当前客户端会请求：

```text
POST /chat/completions
```

## 项目结构

```text
src/
  types.ts               # 内部 ModelResponse / ToolCall / message 协议
  agent/
    AgentRunner.ts       # 最小 agent loop runner
    createAgent.ts       # agent facade
    hooks/               # session/tool 生命周期 hook 和审计日志
    memory/              # 长期 memory 存储、摘要和注入
    prompts/
      main.ts            # 主 agent 系统提示词
    run/                 # agent event stream 类型和结果收集
    permissions/         # allow / ask / deny 权限决策和 bash 分类
    subagents/           # subagent markdown 配置加载和 registry
    utils/
      messages.ts        # assistant/tool message 转换
  cli/
    inputEditor.ts       # TTY raw-mode 输入编辑器
    terminal.ts          # 终端输入输出循环
    renderers/           # compact / verbose / off 输出渲染器
    utils/
      keys.ts            # Enter / modified Enter 键位识别
      displayWidth.ts    # 输入渲染宽度估算
  config/
    localModelConfig.ts  # 模型 provider 配置读取
    envFile.ts           # 本地 .env 加载
  model/
    localModelClient.ts  # OpenAI-compatible 模型客户端
    utils/
      openAICompatible.ts # OpenAI-compatible 工具格式转换
      response.ts        # provider 响应归一化
  tools/
    defaultTools.ts      # 默认工具集合
    memory/              # remember 长期记忆写入工具
    core/
      ToolRegistry.ts    # 工具注册和分发
      pathSandbox.ts     # 工作区路径沙箱
      types.ts           # ToolDefinition / ToolSchema
    discovery/
      discoveryTools.ts  # list_dir / glob / grep / web_fetch 只读检索工具集合
      listDirTool.ts     # 目录结构枚举
      globTool.ts        # 文件路径模式匹配
      grepTool.ts        # 代码内容检索（ripgrep）
      webFetchTool.ts    # 网页读取与正文提取
      schemas.ts         # discovery 工具 schema
    bash/
      bashTool.ts        # bash 工具核心逻辑
      schema.ts          # bash schema
      utils/
        command.ts       # 子进程执行和结果格式化
    filesystem/
      filesystemTools.ts # read/write/edit 文件工具核心逻辑
      schemas.ts         # 文件工具 schema
      utils/
        input.ts         # 文件工具输入校验
        text.ts          # 文本截断和行数限制
    todo/
      todoTool.ts        # todo 工具核心逻辑
      schema.ts          # todo schema
      state.ts           # 任务状态内存管理
      types.ts           # TodoItem / PlanState
    task/
      taskTool.ts        # task / subagent 委托工具
      schema.ts          # task schema
    skills/
      loadSkillTool.ts   # load_skill 技能按需加载工具
      schema.ts          # load_skill schema
  index.ts               # CLI 入口
subagents/               # Markdown subagent 配置
skills/                  # skills/*/SKILL.md 技能说明文件
docs/
  learning/
    sessions/            # 每日实践记录
test/
  agent-runner.test.ts
  cli-keys.test.ts
  config.test.ts
  env-file.test.ts
  todo.test.ts
  tools.test.ts
```

## Skills 约定

- 入口文件：`skills/<skill-name>/SKILL.md`
- 可选参考资料：
  - `skills/<skill-name>/reference.md`
  - `skills/<skill-name>/references/**`
- `load_skill` 支持：
  - `action: "list"`：列出可用 skills
  - `action: "load"`：加载 skill 指令正文
  - `action: "list_references"`：列出 skill 的 reference 文件
  - `action: "load_reference"`：加载指定 reference 内容
  - `action: "list_scripts"`：列出 skill 下可执行脚本
- `run_skill` 支持：
  - 仅执行 `skills/<name>/scripts/**` 下已注册的 Node 脚本（`.js/.mjs/.cjs`）
  - 可选 `args` 与 `timeout_ms`

## 后续重写路线

1. 阅读你提供的网站文档和 Python 实现，整理概念映射。
2. 先重写最小 agent loop：输入、规划、模型调用、输出。
3. 加入 tool registry 和 tool calling。
4. 加入 terminal / filesystem 等可控工具能力。
5. 加入上下文压缩、任务状态、错误恢复。
6. 用完整代码过程反推 agent 的设计模型。

## 学习路线

后续实践会参考 `shareAI-lab/learn-claude-code` 和官网 19 章主题推进。我们已经把逐主题实践路线、主题索引、差异分析和每日记录模板沉淀在：

- [docs/learning/README.md](./docs/learning/README.md)
- [docs/learning/source-gap-analysis.md](./docs/learning/source-gap-analysis.md)
- [docs/learning/model-provider-plan.md](./docs/learning/model-provider-plan.md)
- [docs/learning/model-response-shape.md](./docs/learning/model-response-shape.md)
- [docs/learning/topic-index.md](./docs/learning/topic-index.md)
- [docs/learning/two-week-plan.md](./docs/learning/two-week-plan.md)
- [docs/learning/session-template.md](./docs/learning/session-template.md)
